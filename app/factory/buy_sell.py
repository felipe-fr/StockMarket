...
def buy_shares(form, name):
    ...
    if not price:
        ...
        if user.cash > cost:
            ...
            user.cash = cash
            db.session.commit()
            new_buy = TransactionHistory(user_id=user.id, symbol=(form.symbol.data).upper(
            ), shares=shares, price=price, cost=cost, date_time=date_time)
...
def sell_shares(form, name):
    if sell_validation(form, name):
        ...
        user.cash = cash
        db.session.commit()
        new_sell = TransactionHistory(user_id=user.id, symbol=(form.symbol.data).upper(
        ), shares=shares, price=price, cost=gain, date_time=date_time)
